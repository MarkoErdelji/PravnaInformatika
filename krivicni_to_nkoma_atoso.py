#!/usr/bin/env python3
# parse_kz.py
# Usage: python parse_kz.py input.html output.xml
...
# -------------------------
SERBIAN_NUMBER_MAP = {
    'PRVA':1,'DRUGA':2,'TREĆA':3,'ČETVRTA':4,'PETA':5,'ŠESTA':6,'SEDMA':7,'OSMA':8,'DEVETA':9,'DESETA':10,
    'JEDANAESTA':11,'DVANAESTA':12,'TRINAESTA':13,'ČETRNAESTA':14,'PETNAESTA':15,'ŠESNAESTA':16,'SEDAMNAESTA':17,'OSAMNAESTA':18,
    'DEVETNAESTA':19,'DVADESET':20,'DVADESET PRVA':21,'DVADESET DRUGA':22,'DVADESET TREĆA':23,'DVADESET ČETVRTA':24,
    'DVADESET PETA':25,'DVADESET ŠESTA':26,'DVADESET SEDMA':27,'DVADESET OSMA':28,'DVADESET DEVETA':29,'TRIDESET':30,
    'TRIDESET PRVA':31,'TRIDESET DRUGA':32,'TRIDESET TREĆA':33,'TRIDESET ČETVRTA':34,'TRIDESET PETA':35,'TRIDESET ŠESTA':36
}

def serbian_to_int(r):
    if not r:
        return None
    r = r.strip().upper()
    return SERBIAN_NUMBER_MAP.get(r)
        ...
        if next_type == "existing":
            ...
            if m.group("artA"):  # člana N st. X,Y i Z tač. A,B
                ...
                for stav in stav_list:
                    hrefs.append((f"/krivicni#art_{art}_st_{stav}", f"člana {art} st. {stav}"))
                ...
                art = m.group("artB")
                stav = m.group("stavB")
                hrefs.append((f"/krivicni#art_{art}_st_{stav}", full))
            elif m.group("stavD"):  # stava X,Y i Z tač. A,B ovog člana
                stav_list = split_stav_list(m.group("stavD"))
                for stav in stav_list:
                    href = f"/krivicni#{current_article_eid}_st_{stav}" if current_article_eid else f"/krivicni#art_unknown_st_{stav}"
                ...
                art = m.group("artG")
                hrefs.append((f"/krivicni#art_{art}", full))
...
def parse_html_to_akn(html_text):
    ...
    for p in p_elems:
        ...
        # Part detection (OPŠTI DIO, POSEBNI DIO)
        if text.upper() in ["OPŠTI DIO", "POSEBNI DIO"]:
            part_num = 1 if text.upper() == "OPŠTI DIO" else 2
            current_part = ET.SubElement(body, "part", eId=f"part_{part_num}")
            heading_elem = ET.SubElement(current_part, "heading")
            heading_elem.text = text
            current_container = current_part
            current_chapter = None
            current_section = None
            next_article_heading = None
            continue

        m_glava = re.match(r"Glava\s+([A-ZČĆŠĐŽ\s]+)", text, flags=re.IGNORECASE)
        if m_glava:
            serbian = m_glava.group(1).upper()
            num = serbian_to_int(serbian) or serbian
            current_chapter_id = f"chap_{num}"
            current_chapter = ET.SubElement(current_container, "chapter", eId=current_chapter_id)
            num_elem = ET.SubElement(current_chapter, "num")
            num_elem.text = f"GLAVA {serbian}"
            current_section = None
            next_article_heading = None
            continue

        if current_chapter is None:
            current_chapter = ET.SubElement(current_container, "chapter", eId="chap_unknown")
        ...
        if "clan" in cls.lower() or m_clan:
            ...
            current_article = ET.SubElement(parent, "article", eId=f"art_{art_num}")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python parse_kz.py input.html output.xml")
        sys.exit(1)
    in_path = sys.argv[1]
    out_path = sys.argv[2]
